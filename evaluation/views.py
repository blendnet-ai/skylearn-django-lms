...
from evaluation.seriailzers import DSAChatHistoryQueryParamsSerializer, QuestionIssuesSerializer
User = get_user_model()
...
from .repositories import QuestionRepository, AssessmentGenerationConfigRepository, AssessmentAttemptRepository, UserEvalQuestionAttemptRepository
from .usecases import AssessmentUseCase, DSAChatHistoryUseCase, EvaluationUseCase, MockInterviewReportUsecase, XobinUseCase, DSAPracticeUsecase, DSAReportUsecase, DSAQuestionsUsecase, QuestionIssueUsecase, DSAPracticeReportHistoryUsecase,DashBoardDetailsUsecase, DSASheetsConfigUsecase, AssessmentExpiredException
...
class StartAssessment(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not assessment_generation_id:
            return Response({"error": "Assessment Generation ID not provided"}, status=status.HTTP_400_BAD_REQUEST)
        try:
            generated_assessment_data = AssessmentUseCase.create_new_assessment(assessment_generation_id, user, assessment_generation_details)
...
class GetNextQuestion(APIView):
    ...
    def get(self, request, format=None):
        ...
        if question_id == 0:
            return Response({"error": "Assessment completed"}, status=status.HTTP_200_OK)
        if not question_id:
            return Response({"error": "Next question fetching failed"}, status=status.HTTP_400_BAD_REQUEST)

        assessment = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user_id)
        if not assessment:
            return Response({"error": "Invalid assessment_attempt"}, status=status.HTTP_400_BAD_REQUEST)

        question_data = AssessmentUseCase.fetch_question_data(question_id,assessment, user_id)
...
class FetchQuestion(APIView):
    ...
    def get(self, request, format=None):
        ...
        assessment_question_check = AssessmentUseCase.check_if_question_exists_in_assessment(assessment_id, question_id, user_id)
        if not assessment_question_check:
            return Response({"error": "Question not in assessment"}, status=status.HTTP_404_NOT_FOUND)
        
        question_data = AssessmentUseCase.fetch_question_data(question_id,assessment, user)
        if not question_data:
            return Response({"error": "Invalid question id"}, status=status.HTTP_400_BAD_REQUEST)
...
class FetchAssessmentData(APIView):
    ...
    def get(self, request, format=None):
        ...
        assessment_data = AssessmentAttemptRepository.fetch_assessment_questions(assessment_id, user_id)
...
class FetchAssessmentState(APIView):
    ...
    def get(self, request, format=None):
        ...
        assessment_state = AssessmentAttemptRepository.fetch_assessment_state(assessment_id, user_id)
        if not assessment_state:
            return Response({"error": "Invalid assessment_attempt"}, status=status.HTTP_400_BAD_REQUEST)

        if 'attempted_questions' in assessment_state and assessment_state['attempted_questions']:
            for question in assessment_state['attempted_questions']:
                if question.get('code_stubs'):
                    question_id = question['question_id']
                    code_stubs = DSAQuestionsUsecase.get_code_stubs(question_id, assessment_id, request.user)
...
class FetchIndividualScorecard(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            scorecard = AssessmentUseCase.fetch_assessment_scorecard_by_id(assessment_id)
...
class SubmitAssessmentAnswerMCQ(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            EvaluationUseCase.save_question_attempt(assessment_id, user, question_id, section, 'mcq_answer', mcq_answer)
        except AssessmentExpiredException as e:
            return Response({"error": str(e)}, status=status.HTTP_410_GONE)
...
class SubmitAssessmentAnswerMMCQ(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            EvaluationUseCase.save_question_attempt(assessment_id, user, question_id, section, 'multiple_mcq_answer', multiple_mcq_answer)            
        except AssessmentExpiredException as e:
            return Response({"error": str(e)}, status=status.HTTP_410_GONE)
...
class SubmitAssessmentAnswerSubjective(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            EvaluationUseCase.save_question_attempt(assessment_id, user, question_id, section, 'answer_text', answer_text)      
        except AssessmentExpiredException as e:
            return Response({"error": str(e)}, status=status.HTTP_410_GONE)
...
class SubmitAssessmentAnswerVoice(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            EvaluationUseCase.save_question_attempt(assessment_id, user, question_id, section, 'voice')    
        except AssessmentExpiredException as e:
            return Response({"error": str(e)}, status=status.HTTP_410_GONE)
...
class CloseAssessment(APIView):    
    ...
    def post(self, request, format=None):
        ...
        assessment = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user)
        if not assessment:
            return Response({"error": "Invalid assessment_attempt"}, status=status.HTTP_400_BAD_REQUEST)
...
class XobinResultWebhook(APIView):
    ...
    def post(self, request, format=None):
        ...
        if XobinUseCase.verify_webhook(data, hmac_header):
            ...
            return Response({"data": "Webhook received and verified"}, status=status.HTTP_200_OK)
        else:
...
class DSAExecute(APIView):
    ...
    def post(self, request, format=None):
        ...
        if type_of_evaluation == 'run':
            ...
            DSAPracticeUsecase.call_executor(user, question_id, assessment_id, type_of_evaluation, language, code,
                                             custom_testcases)
            ...
            return Response({"error": "Invalid Submit Type"}, status=status.HTTP_400_BAD_REQUEST)
...
class DSAExecutionStatus(APIView):
    ...
    def get(self, request,format=None):
        ...
        data=DSAPracticeUsecase.get_execution_status(user=user,assessment_id=assessment_id,question_id=question_id)
...
class DSAReport(APIView):
    ...
    def get(self, request, _=None):
        ...
        attempt = AssessmentAttemptRepository.fetch_assessment_attempt(assessment_attempt_id)

        if not attempt:
            return Response({"error": "Assessment attempt not found"}, status=status.HTTP_404_NOT_FOUND)

        if not attempt.question_list or not attempt.question_list[0]["questions"]:
            return Response({"error": "No questions found in the assessment attempt"}, status=status.HTTP_404_NOT_FOUND)
        
        question_id = attempt.question_list[0]["questions"][0]
        
        question = QuestionRepository.fetch_question(question_id)
        if not question:
            return Response({"error": "Question details not found"}, status=status.HTTP_404_NOT_FOUND)
        
        report=DSAReportUsecase.generate_report(user=request.user,attempt=attempt,question=question)      
...
class BaseDSAQuestionsList(APIView):
    ...
    def get_dsa_questions(self, request, is_lab=False):
        ...
        return Response({"data": dsa_questions_response.__dict__}, status=status.HTTP_200_OK)
...
class DSAPracticeAttemptGenerator(APIView):
    ...
    def get(self, request):
        ...
        generated_assessment_data=DSAQuestionsUsecase.generate_attempt_by_question_id(user,questions_id, mode)
        return Response({"data": generated_assessment_data}, status=status.HTTP_200_OK)
...
class QuestionIssueReport(APIView):
    ...
    def post(self, request):
        ...
        question_issue_id = QuestionIssueUsecase.create_issue(
            user=user, 
            assessment_attempt_id=question_serializer.validated_data['assessment_attempt_id'],
...
class DSAPracticeReportHistory(APIView):
    ...
    def get(self, request):
        user = request.user
        practice_history = DSAPracticeReportHistoryUsecase.get_dsa_reports(user_id=user.id)
...
class DSASheetList(APIView):
    ...
    def get(self, request):
        ...
        sheet_questions_response = DSASheetsConfigUsecase.get_sheet_questions(user=user, sheet_id=sheet_id)
        sheet_status_response = DSASheetsConfigUsecase.get_sheet_status(user=user, sheet_id=sheet_id)
        return Response({"data": {**sheet_questions_response.__dict__, "sheet_status": sheet_status_response}}, status=status.HTTP_200_OK)
...
class DSAAssessmentChatHistoryView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        if request.user.is_staff:
            chat_data =DSAPracticeChatDataRepository.get_chat_data_by_assessment_id(assessment_id=assessment_id)
            return render(request, 'chat_history.html', {'chat_data': chat_data, 'assessment_id': assessment_id})
        else:
...
class MockInterviewBehaviouralReportView(APIView):
    ...
    def get(self, request, _=None):
        ...
        report=MockInterviewReportUsecase.generate_behavioural_report(user=request.user,attempt=attempt)      
