...
from OpenAIService.repositories import LLMCommunicationWrapper, ValidPromptTemplates, ChatHistoryRepository
from ai_learning.repositories import DSAPracticeChatDataRepository, PromptTemplateRepository, CompileDurationLog
...
from evaluation.repositories import AssessmentAttemptRepository, AssessmentGenerationConfigRepository, \
    QuestionRepository, UserEvalQuestionAttemptRepository, AudioURLProvider, QuestionIssuesRepository, \
...
class QuestionsData:
    ...
    def validate_data(self):
        ...
        if not all(isinstance(scq, SectionWiseQuestions) for scq in self.questions):
            raise ValueError("All section wise questions must be instances of SectionWiseQuestions.")
        if not self.questions:
...
class AssessmentUseCase:
    ...
    def fetch_azure_storage_url(blob_name, container_name):
        ...
        return az_storage_service.generate_blob_access_url(container_name=container_name, blob_name=blob_name,
    ...
    def fetch_display_data(user_id):
        ...
        for assessment in available_assessments:
            ...
            if assessment_generation_id == int(AssessmentAttempt.Type.CODING) + 1 and str(
                    user_id) not in settings.USER_IDS_CODING_TEST_ENABLED:
                continue
    ...
    def fetch_attempts_data(user_id):
        available_assessment_configs = AssessmentAttemptRepository.fetch_assessment_configs()
        current_assessments = AssessmentAttemptRepository.fetch_all_assessment_attempt(user_id)
        ...
        for assessment_config in available_assessment_configs:
            ...
            assessment_attempts = current_assessments.filter(type=assessment_config.kwargs.get('category')).order_by(
                '-created_at').all()
            ...
            if assessment_attempts:
                ...
                if assessment_attempts[0].closed:
                    if latest_attempt_status == int(AssessmentAttempt.Status.COMPLETED) and assessment_attempts[
                        0].evaluation_triggered:
    ...
    def fetch_question_data(question_id, assessment, user, for_exeution=False):
        ...
        if question.question_data.get('questionTitle'):
            resp_question_data['questionTitle'] = question.question_data.get('questionTitle')
        ...
        if question.question_data.get('exampleTestcases'):
            resp_question_data['exampleTestcases'] = question.question_data.get('exampleTestcases')

        if question.question_data.get('additionalTestcases') and for_exeution:
            resp_question_data['additionalTestcases'] = question.question_data.get('additionalTestcases')

        if question.question_data.get('driver_codes') and for_exeution:
            resp_question_data['driver_codes'] = question.question_data.get('driver_codes')
        ...
        if question.answer_type == int(Question.AnswerType.VOICE.value):
            answer_audio_url = EvaluationUseCase.save_audio_url_speaking(user, question_id, assessment)
    ...
    def assert_assessment_validity(start_time, test_duration, status):
        ...
        if status == int(AssessmentAttempt.Status.ABANDONED) or status == int(
                AssessmentAttempt.Status.COMPLETED) or status == int(AssessmentAttempt.Status.EVALUATION_PENDING):
    ...
    def fetch_assessment_data(assessment_id, user_id):
        assessment_details = AssessmentAttemptRepository.get_assessment_data(assessment_id=assessment_id,
                                                                             user_id=user_id)
    ...
    def fetch_assessment_data_and_assert_validity(assessment_id, user_id):
        assessment_details = AssessmentUseCase.fetch_assessment_data(assessment_id=assessment_id, user_id=user_id)
        AssessmentUseCase.assert_assessment_validity(assessment_details.start_time, assessment_details.test_duration,
                                                     assessment_details.status)

        return assessment_details

    def fetch_next_question_id(assessment_id, user_id):
        assessment_details = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user_id)

        if assessment_details:
            last_saved = int(assessment_details.last_saved) if assessment_details.last_saved else None
            question_list = assessment_details.question_list
            last_saved_section = int(
                assessment_details.last_saved_section) if assessment_details.last_saved_section else None
            ...
            if last_saved_section is not None:
                for item in question_list:
                    if item.get('section') == last_saved_section:
                        if last_saved in item.get('questions'):
                            index_of_last_saved = item.get('questions').index(last_saved)
                            if index_of_last_saved + 1 < len(item.get('questions')):
                                next_question = item.get('questions')[index_of_last_saved + 1]
                                break
                            else:
                                index_of_last_saved_section = question_list.index(item)

                                if index_of_last_saved_section + 1 < len(question_list):
                                    next_question = question_list[index_of_last_saved_section + 1].get('questions')[0]
                                    break
                                else:
                                    next_question = 0
                                    break
                ...
                if isinstance(question_list, list) and all(isinstance(item, dict) for item in question_list):
    ...
    def fetch_question_ids_from_tag(category, total_questions_count, tags, section_name, skippable):
        ...
        for item in tags:
            ...
            questions = QuestionRepository.return_questions_by_tag(category, num_questions, tag, selected_question_ids)
        ...
        if questions_injestion_left > 0:
            remaining_questions = QuestionRepository.return_questions_by_category(category, questions_injestion_left,
                                                                                  selected_question_ids)
            selected_question_ids.extend([question.id for question in remaining_questions])
    ...
    def fetch_question_ids_from_sub_category(category, total_questions_count, subcategories):
        ...
        for item in subcategories:
            ...
            questions = QuestionRepository.return_questions_by_sub_category(category, sub_category, num_questions,
                                                                            selected_question_ids)
            selected_question_ids.extend([question.id for question in questions])

            # fetching existing section if there is same section name else create new section
            existing_section = next(
                (section for section in sub_category_wise_questions if section.section == section_name), None)

            if existing_section:
                existing_section.questions.extend([question.id for question in questions])
            else:
    ...
    def check_if_question_exists_in_assessment(assessment_id, question_id, user_id):
        assessment = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user_id)
        if assessment:
    ...
    def create_new_assessment(assessment_generation_id, user, assessment_generation_details):
        ...
        if class_name == "MockInterviewBasedRandomAssessment" and assessment_generation_details is not None:
            assessment_creation_instance = assessment_class(assessment_generation_id,assessment_generation_details)
        elif class_name == "MockInterviewBasedRandomAssessment" and  assessment_generation_details is None:
            raise ValueError("Assessment generation details required in body for mock interview based assessment generation")
        else:
            assessment_creation_instance = assessment_class(assessment_generation_id)


        generated_assessment_data = assessment_creation_instance.generate_assessment_attempt(user)

        if generated_assessment_data.get('assessment_url'):
            start_time = timezone.now()
            assessment_id = AssessmentAttemptRepository.add_or_update_assessment_attempt(start_time=start_time,
                                                                                         user=user,
                                                                                         test_duration=test_duration,
                                                                                         assessment_generation_id=assessment_generation_class_data,
                                                                                         assessment_url=generated_assessment_data.get(
                                                                                             'assessment_url'),
                                                                                         report_id=generated_assessment_data.get(
                                                                                             'report_id'),
                                                                                         status=AssessmentAttempt.Status.IN_PROGRESS,
                                                                                         type=category)
        else:
            assessment_id = AssessmentAttemptRepository.add_or_update_assessment_attempt(test_duration=test_duration,
                                                                                         assessment_generation_id=assessment_generation_class_data,
                                                                                         type=generated_assessment_data[
                                                                                             'category'],
                                                                                         status=AssessmentAttempt.Status.IN_PROGRESS,
                                                                                         question_list=
        ...
        mark_test_abandoned.apply_async((assessment_id, user.id), countdown=total_delay_seconds)
    ...
    def exit_assessment(assessment):
        AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment_attempt=assessment, closed=True,
    ...
    def fetch_history_data(user_id):
        ...
        filter_options = []
        for assessment in available_assessments:
            if assessment.assessment_generation_id == int(AssessmentAttempt.Type.CODING) + 1 and str(
                    user_id) not in settings.USER_IDS_CODING_TEST_ENABLED:
                continue
...
class EvaluationUseCase:
    ...
    def save_audio_url_speaking(user, question_id, assessment):
        ...
        if question.answer_type == int(Question.AnswerType.VOICE.value):
            user_question_attempt = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question_id,
                                                                                                  assessment)

            if user_question_attempt and user_question_attempt.answer_audio_url:
                return user_question_attempt.answer_audio_url

            user_question_attempt = UserEvalQuestionAttemptRepository.create_user_question_attempt(user, question_id,
                                                                                                   assessment)
            az_storage_service = AzureStorageService()
            audio_path = user_question_attempt.audio_path
            container_name = AudioURLProvider.get_storage_container_name()
            answer_audio_url = az_storage_service.generate_blob_access_url(container_name=container_name,
                                                                           blob_name=audio_path,
                                                                           expiry_time=datetime.now() + timedelta(
                                                                               days=1), allow_read=True,
                                                                           allow_write=True)
            UserEvalQuestionAttemptRepository.save_user_question_attempt(user_question_attempt,
                                                                         answer_audio_url=answer_audio_url)

            return answer_audio_url

    def save_question_attempt(assessment_id, user, question_id, section, answer_type=None, answer=None):

        assessment_attempt = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user.id)
        user_attempt = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question_id,
                                                                                     assessment_attempt)
        if not user_attempt:
            user_attempt = UserEvalQuestionAttemptRepository.create_user_question_attempt(user, question_id,
                                                                                          assessment_attempt)

        kwargs = {}
        if answer_type is not None and answer is not None:
            kwargs[answer_type] = answer
        UserEvalQuestionAttemptRepository.save_user_question_attempt(user_attempt,
                                                                     status=UserEvalQuestionAttempt.Status.ATTEMPTED,
                                                                     **kwargs)

        if section is not None:
            UserEvalQuestionAttemptRepository.save_user_question_attempt(user_attempt, section=section)
            AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment_attempt, last_saved_section=section)

        if user_attempt.id not in assessment_attempt.attempted_list:
            AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment_attempt, last_saved=question_id,
                                                                         add_to_attempted_list=user_attempt.id)

        return

    def evaluate_all_questions(assessment):
        AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment, closed=True,
                                                                     status=AssessmentAttempt.Status.EVALUATION_PENDING)
        if assessment:
            attempted_questions_list = UserEvalQuestionAttemptRepository.fetch_attempted_questions(assessment)
            for user_attempt in attempted_questions_list:
                ...
                if user_attempt.mcq_answer is not None:
                    ...
                    num_test_cases_passed = sum(1 for test_case in test_cases_data if test_case.get('passed', False))
                    ...
                    AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment,
                                                                                 eval_data=test_cases_eval_data)
                evaluator_class_name = evaluator_mapping.get(answer_type)
                if evaluator_class_name:
                    evaluator_module = importlib.import_module(f"evaluation.evaluators.{evaluator_class_name}")
...
class XobinUseCase:
    ...
    def generate_xobin_assessment_url(user, assessment_id):
        ...
        user_details = UserProfileRepository.fetch_user_data(user.id).get('entire_data').first()
    ...
    def save_xobin_result(data):
        ...
        assessment = AssessmentAttemptRepository.fetch_assessment_from_report_id(report_id)
        ...
        logger.info(f"Saving Xobin result for assessment {assessment.assessment_id} - {eval_data}")
        AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment, closed=True,
                                                                     status=AssessmentAttempt.Status.COMPLETED,
                                                                     eval_data=eval_data, evaluation_triggered=True)
...
class XobinAssessmentService(BaseRestService):
    ...
    def invite_candidate(self, name: str, email: str, assessment_id: str, expiry_time: str):
        ...
        response = self._post_request(url=api_url, data=request_body, custom_headers=self.get_base_headers())
...
class DSAPracticeUsecase:
    ...
    @staticmethod
    def save_run_execution_log(*, user_id: int, question_id: int, assessment_id: int, duration: float, submit=False):
        c_log = CompileDurationLog(timestamp=int(datetime.now().timestamp()), duration=duration)
        DSAPracticeChatDataRepository.save_run_log(compile_duration_log=c_log
                                                   , user_id=user_id, question_id=question_id,
                                                   assessment_attempt_id=assessment_id, submit=submit)

    @staticmethod
    def call_executor(user, question_id, assessment_id, type_of_evaluation, language, code, custom_testcases):
        assessment_attempt_id = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user.id)
        user_attempt = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question_id,
                                                                                     assessment_attempt=assessment_attempt_id)
        code_stubs = user_attempt.code_stubs
        code_stubs[language] = code
        UserEvalQuestionAttemptRepository.save_user_question_attempt(user_attempt, code=code, code_stubs=code_stubs)
        ...
        if user_attempt.id not in assessment_attempt_id.attempted_list:
            AssessmentAttemptRepository.add_or_update_assessment_attempt(assessment_attempt=assessment_attempt_id,
                                                                         last_saved=question_id,
                                                                         add_to_attempted_list=user_attempt.id)
        question_data = AssessmentUseCase.fetch_question_data(question_id, assessment_attempt_id, user,
                                                              for_exeution=True)
        if (type_of_evaluation == "run"):
            ...
            a = datetime.now()
            DSAPracticeExecutor(user_attempt, test_cases, language, code, main_code).evaluate()
            DSAPracticeUsecase.save_run_execution_log(user_id=user.id, question_id=question_id,
                                                      assessment_id=assessment_id,
            ...
            DSAPracticeUsecase.save_run_execution_log(user_id=user.id, question_id=question_id,
                                                      assessment_id=assessment_id,
                                                      duration=(datetime.now() - a).total_seconds(), submit=True)

    @staticmethod
    def get_execution_status(user, assessment_id, question_id):
        assessment = AssessmentUseCase.fetch_assessment_data_and_assert_validity(assessment_id, user.id)
        user_question_attempt = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question_id,
                                                                                              assessment)
...
class DSAChatHistoryUseCase:
    ...
    @classmethod
    def get_msg_text_and_delay_for_proactive_msg(cls, user_id:int)-> typing.Tuple[str, int]:
        ...
        message_text = config["repeat" if has_previously_completed_assessments else "first"]
        delay = config["delay"]
        return message_text,2

    @classmethod
    def get_chat_history(cls,is_superuser, user_id, question_id, assessment_attempt_id):
        ...
        if user_data.chat_history_obj is None:
            message_text,delay = cls.get_msg_text_and_delay_for_proactive_msg(user_id=user_id)
            ...
            lis=LLMCommunicationWrapper.get_processed_chat_messages(chat_history=user_data.chat_history_obj.chat_history,is_superuser=is_superuser)
            return lis
...
class DSAReportUsecase:
    def get_total_score(eval_data):
        total_score=eval_data.get('code_quality_score', {}).get('Score', 0)+eval_data.get('code_efficiency_score', {}).get('Score', 0) +eval_data.get('code_correctness_score', {}).get('code_correctness_score', 0)
        return total_score
    ...
    def generate_report(user, attempt, question):
        ...
        total_test_cases = eval_data.get('code_correctness_score', {}).get('total_test_cases', 0)
        test_cases_passed = eval_data.get('code_correctness_score', {}).get('num_test_cases_passed', 0)
        test_cases_data = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question.id, attempt).eval_data.get('test_cases',[])
        failed_test_cases = [test_case for test_case in test_cases_data if test_case.get('passed') is False or test_case.get('error_type') is not None]
        failed_test_cases = failed_test_cases[:10] if len(failed_test_cases) > 10 else failed_test_cases
        submitted_code = UserEvalQuestionAttemptRepository.fetch_user_question_attempt(user, question.id, attempt).code
        # Check if all required scores are present
        scores_available = (
            eval_data.get('code_correctness_score', {}).get('code_correctness_score') is not None and
            eval_data.get('code_efficiency_score', {}).get('Score') is not None and
            eval_data.get('code_quality_score', {}).get('Score') is not None
        )

        detailed_report=True if total_test_cases !=0 and eval_data.get('code_correctness_score', {}).get('code_correctness_score', 0) > 10 else False
        
        report = {
            "top": {
                "student_name": full_name,
                "date_of_session": str(attempt.updated_at.date()),
                "title_of_the_dsa_problem": question.question_data.get('questionTitle', ''),
                "difficulty_level_and_tags": question.question_data.get('difficulty', '')
            },
            "total_score": {
                "score": DSAReportUsecase.get_total_score(eval_data) if scores_available else None,
                "overall_feedback": eval_data.get('code_summary', {}).get('OverallSummary', None)
            },
            "detailed_performance_analysis": {
                "correctness": {
                    "score": eval_data.get('code_correctness_score', {}).get('code_correctness_score', 0) if total_test_cases > 0 else None,
                    "feedback": f"Test cases passed: {test_cases_passed}/{total_test_cases}" if total_test_cases > 0 else None,
                    "failed_tests": failed_test_cases if len(failed_test_cases) > 0 else None,
                },
                "report_not_generated_reasoning": "Not enough test cases passed to generate detailed feedback. Please review solution and re-attempt the question" if not detailed_report else None,
                "efficiency": {
                    "score": eval_data.get('code_efficiency_score', {}).get('Score', None),
                    "time_complexity": eval_data.get('code_efficiency_score', {}).get('timecomplexity', None),
                    "space_complexity": eval_data.get('code_efficiency_score', {}).get('spacecomplexity', None),
                    "optimum_time_complexity": eval_data.get('code_efficiency_score', {}).get('optimumtimecomplexity', None)
                },
                "code_quality": {
                    "score": eval_data.get('code_quality_score', {}).get('Score', 0),
                    "code_readability": eval_data.get('code_quality_score', {}).get('Feedback', {}).get('CodeReadabilityBestPractices', None),
                    "variable_naming": eval_data.get('code_quality_score', {}).get('Feedback', {}).get('VariableNaming', None),
                    "code_structure": eval_data.get('code_quality_score', {}).get('Feedback', {}).get('CodeStructure', None),
                    "usage_of_comments": eval_data.get('code_quality_score', {}).get('Feedback', {}).get('UsageOfComments', None)
                },
                "improvement_and_learning": {
                    "score": 0,
                    "feedback": eval_data.get('code_improvement_score', {}).get('Feedback', None)
                },
            },
            "session_insights": {
                "key_strengths": eval_data.get('code_summary', {}).get('StrongPoints', None),
                "areas_for_improvement": eval_data.get('code_summary', {}).get('AreaOfImprovements', None)
            },
            "revision_topics": eval_data.get('code_revision_topics', None),
            "resources": {
                "article_link": question.question_data.get('resources', {}).get('article_link', ''),
                "video_link": question.question_data.get('resources', {}).get('video_link', ''),
            },
...
class DSABotWebsocketConsumerUseCase:
    ...
    @staticmethod
    def _call_gpt(user_input, conversational_history, code, question, language, run_result, name):
        llm = OpenAIService()

        prompt = DSABotWebsocketConsumerUseCase._substitue_in_prompt(code, question, language, run_result, name)
        ...
        conversational_history_for_gpt = DSABotWebsocketConsumerUseCase.convert_chat_history_to_openai_spec(
            conversational_history, remove_code=True
        )

        messages.extend(conversational_history_for_gpt)
        messages.append({"role": "user", "content": user_input + prompt_template.additional_guardrail_prompt})

        logger.info(f"Sending final msgs: {messages}")
        completion = llm.get_completion_from_messages(messages, llm_config_name_options=["dsa_chat_gpt_1", "dsa_chat_gpt_2"])

        return completion

    @staticmethod
    def convert_chat_history_to_openai_spec(conversational_history: List, remove_code=False) -> typing.List[typing.Dict]:
        to_return = []
    ...
    @staticmethod
    def handle_request_2(user_input, user_id, question_id, assessment_attempt_id, code, language, run_result):
        ...
        if dsa_practice_data.chat_history_obj is None:
            llm_wrapper = LLMCommunicationWrapper(prompt_name=ValidPromptTemplates.DSA_PRACTICE,
                                    chat_history_id=None,initialize=True,initializing_context_vars=context)
            message_text, _ = DSAChatHistoryUseCase.get_msg_text_and_delay_for_proactive_msg(user_id=user_id)
            ...
            DSAPracticeChatDataRepository.add_chat_history_id(
                dsa_practice_data=dsa_practice_data, chat_history_id=llm_wrapper.get_chat_history_object().id
            )
        else:
            llm_wrapper = LLMCommunicationWrapper(prompt_name=ValidPromptTemplates.DSA_PRACTICE,
        ...
        response_text = llm_wrapper.send_user_message_and_get_response(user_input,context)
        return response_text

    @staticmethod
    def handle_request(
            user_input, user_id, question_id, assessment_attempt_id, code, language, run_result
    ):
        ...
        try:
            ...
            response = DSABotWebsocketConsumerUseCase._call_gpt(
                user_input, conversational_history, code, question_text, language, run_result, name
            )
...
class DSAQuestionsUsecase:
    ...
    def _get_lab_question_list(user, solved_question_ids):
        ...
        questions = QuestionRepository.return_questions_by_category(int(Question.Category.DSA_PRACTICE), None, [])
        return questions

    @staticmethod
    def get_question_list(user, lab=False):
        successful_assessment_attempts = AssessmentAttemptRepository.fetch_all_assessments_completed(user.id)
        solved_question_ids = {attempt.question_list[0]["questions"][0] for attempt in successful_assessment_attempts}

        if lab:
            questions = DSAQuestionsUsecase._get_lab_question_list(user, solved_question_ids)
        else:
            questions = QuestionRepository.return_questions_by_category(int(Question.Category.DSA_PRACTICE), None, [])
        ...
        for question in questions:
            ...
            if question.source is not None:
                locked = False
            question_data = {"id": question.id, "title": question.question_data["questionTitle"],
                             "difficulty": question.question_data["difficulty"], "topics": question.tags,
                             "companies": question.question_data['companies'], "locked": locked, "score": None}
            ...
            for attempt in successful_assessment_attempts:
                if question.id in attempt.question_list[0]["questions"]:
                    if question.id in solved_question_ids:
                        question_data["score"] = DSAReportUsecase.get_total_score(attempt.eval_data)
                    else:
        ...
        questions_list = sorted(questions_list,
                                key=lambda x: (x['locked'], difficulty_order[x['difficulty'].lower()], x['id']))
        topics_list = sorted(list(topics_set))
        return DSAQuestionsResponse(dsa_sheet_names, questions_list, topics_list, companies_set)

    @staticmethod
    def get_code_stubs(question_id, assessment_id, user):
        assessment_attempt = AssessmentAttemptRepository.get_assessment_data(assessment_id, user.id)
        question_data = AssessmentUseCase.fetch_question_data(question_id, assessment_attempt, user, for_exeution=True)
    ...
    def generate_attempt_by_question_id(user, question_id, mode):
        ...
        assessment_generation_class_data = AssessmentGenerationConfigRepository.return_assessment_generation_class_data(
            5)
        generated_assessment_data = {'total_number': 1, 'category': int(Question.Category.DSA_PRACTICE), 'questions': [
            {"section": question.question_data['difficulty'], "questions": [question.id], "skippable": False}]}

        assessment_id = AssessmentAttemptRepository.add_or_update_assessment_attempt(test_duration=required_time,
                                                                                     assessment_generation_id=assessment_generation_class_data,
                                                                                     type=generated_assessment_data[
                                                                                         'category'],
                                                                                     status=AssessmentAttempt.Status.IN_PROGRESS,
        ...
        generated_assessment_data['assessment_id'] = assessment_id
        user_codes = DSAQuestionsUsecase.get_code_stubs(question_id=question_id, assessment_id=assessment_id, user=user)
        current_assessment = AssessmentAttemptRepository.fetch_assessment_attempt(assessment_id=assessment_id)
        user_attempt = UserEvalQuestionAttemptRepository.create_user_question_attempt(user=user,
                                                                                      question_id=question_id,
                                                                                      assessment_attempt_id=current_assessment)
        UserEvalQuestionAttemptRepository.save_user_question_attempt(user_attempt, status=int(
            UserEvalQuestionAttempt.Status.ATTEMPTED), code_stubs=user_codes)
        return generated_assessment_data


class QuestionIssueUsecase:
    @staticmethod
    def create_issue(user, type_of_issue, question_id, assessment_attempt_id, description):
        report_id = QuestionIssuesRepository.save_question_issue(user=user,
                                                                 type_of_issue=type_of_issue, question_id=question_id,
                                                                 assessment_attempt_id=assessment_attempt_id,
...
class DSAPracticeReportHistoryUsecase:
    ...
    def get_dsa_reports(user_id):
        successful_assessment_attempts = AssessmentAttemptRepository.fetch_all_assessments_completed(user_id)
        all_reports = []
        for attempt in successful_assessment_attempts:
            report_data = {}
            eval_data = attempt.eval_data
...
class DashBoardDetailsUsecase:
    ...
    def _calculate_correctness_score(attempt):
        return attempt.eval_data.get('code_correctness_score', {}).get('code_correctness_score', 0)
    ...
    def _is_problem_solved(attempt):
        correctness_score = DashBoardDetailsUsecase._calculate_correctness_score(attempt)
        return correctness_score >= 40

    @staticmethod
    def get_dashboard_details(user_id):
        # Fetch all completed assessment attempts for the user
        assessment_attempts = AssessmentAttemptRepository.fetch_all_assessments_completed(user_id)
        ...
        question_ids = {attempt.question_list[0]["questions"][0] for attempt in assessment_attempts}
        ...
        chat_sessions = DSAPracticeChatDataRepository.fetch_chat_sessions(user_id, list(attempt_ids))
        chat_sessions_map = {session.assessment_attempt_id: session.llm_chat_count for session in chat_sessions}
        ...
        for attempt in assessment_attempts:
            ...
            code_quality_sum += DashBoardDetailsUsecase._calculate_score_sum(attempt, 'code_quality_score')
            code_efficiency_sum += DashBoardDetailsUsecase._calculate_score_sum(attempt, 'code_efficiency_score')
            code_correctness_sum += DashBoardDetailsUsecase._calculate_correctness_score(attempt)
            ...
            total_time_spent += (attempt.updated_at - attempt.start_time).total_seconds() / 60 if isinstance(
                attempt.updated_at, datetime) and isinstance(attempt.start_time, datetime) else 0

            if is_problem_solved:
                successful_assessment_attempts.append(attempt)

        # Calculate average scores
        code_quality_avg = DashBoardDetailsUsecase._calculate_average_score(code_quality_sum, 20, assessment_attempts)
        code_efficiency_avg = DashBoardDetailsUsecase._calculate_average_score(code_efficiency_sum, 30,
                                                                               assessment_attempts)
        code_correctness_avg = DashBoardDetailsUsecase._calculate_average_score(code_correctness_sum, 50,
                                                                                assessment_attempts)
        ...
        data = {
            ...
            'avg_time_per_question': total_time_spent / len(questions_solved) if questions_solved else 0,
            'success_rate': len(successful_assessment_attempts) / len(assessment_attempts) * 100 if len(
                assessment_attempts) > 0 else 0,
...
class DSASheetsConfigUsecase:
    def get_sheet_questions(user, sheet_id):
        ...
        companies_set = set()
        questions_list = []
        question_ids = DSASheetConfigRepository.get_config_questions_list_by_id(sheet_id)
        if question_ids:
            questions = QuestionRepository.fetch_questions_by_ids(list(question_ids))
        else:
            return DSAQuestionsResponse(dsa_sheet_names, questions_list, topics_set, companies_set)
        ...
        # Sort questions by difficulty and locked status
        questions_list = sorted(questions_list, key=lambda x: (x['locked'], difficulty_order[x['difficulty'].lower()]))
        topics_list = sorted(list(topics_set))
        return DSAQuestionsResponse(dsa_sheet_names, questions_list, topics_list, companies_set)

    def get_sheet_status(user, sheet_id):
        config_question_ids = DSASheetConfigRepository.get_config_questions_list_by_id(sheet_id)
        assessment_attempts = AssessmentAttemptRepository.fetch_all_assessments_completed(user_id=user.id)
        solved_question_ids = {attempt.question_list[0]["questions"][0] for attempt in assessment_attempts}

        solved_config_questions = [qid for qid in config_question_ids if qid in solved_question_ids]

        return {"solved_count": len(solved_config_questions), "total_questions": len(config_question_ids)}
