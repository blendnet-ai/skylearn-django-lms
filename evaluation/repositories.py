...
class QuestionRepository:
    ...
    def return_questions_by_tag(category, num_questions, tag, selected_question_ids):
        questions = (
            Question.objects.filter(category=category, tags__contains=[tag])
    ...
    def return_questions_by_sub_category(
        category, sub_category, num_questions, selected_question_ids
    ):
        questions = (
            Question.objects.filter(category=category, sub_category=sub_category)
            .exclude(id__in=selected_question_ids)
            .order_by("?")[:num_questions]
        )
        return questions
...
class AssessmentGenerationConfigRepository:
    ...
    @staticmethod
    def get_total_assessment_by_course_id(course_id):
...
class AssessmentAttemptRepository:
    ...
    def number_of_attempts_expired(*, assessment_generation_config_id, user_id):
        return AssessmentAttempt.objects.filter(
            assessment_generation_config_id=assessment_generation_config_id,
            user_id=user_id,
            status__in=[
    ...
    def fetch_assessment_state(assessment_id, user_id):
        ...
        attempted_questions = (
            UserEvalQuestionAttemptRepository.fetch_attempted_questions(
                assessment_attempt
            )
        )
    ...
    def fetch_all_assessment_attempts(user_id):
        latest_assessment_attempts = (
            AssessmentAttempt.objects.filter(
                user_id=user_id, status=int(AssessmentAttempt.Status.COMPLETED)
            )
    ...
    def fetch_all_assessments_completed(user_id):
        completed_assessment_attempts = AssessmentAttempt.objects.filter(
            user_id=user_id, type=4, status=int(AssessmentAttempt.Status.COMPLETED)
        ).order_by("type", "-start_time")
    ...
    def does_attempts_exist_for_date(updated_date: timezone.datetime):
        date_to_check = (
            updated_date.date()
            if isinstance(updated_date, timezone.datetime)
            else updated_date
        )

        exists = AssessmentAttempt.objects.filter(
            status=AssessmentAttempt.Status.COMPLETED, updated_at__date=date_to_check
        ).exists()
    ...
    def fetch_all_assessments_for_users(user_ids):
        assessments = AssessmentAttempt.objects.filter(
            user_id__in=user_ids, type=int(Question.Category.DSA_PRACTICE)
        ).all()
        return assessments

    def update_assessment_time(assessment_id, updated_timestamp):
        obj = AssessmentAttempt.objects.filter(
            assessment_id=assessment_id, type=int(Question.Category.DSA_PRACTICE)
        ).first()
    ...
    def fetch_all_assessments_attempts_data():
        assessments = AssessmentAttempt.objects.all()
        return assessments
    
    def fetch_assessments_attempts_data_by_date(date):
        assessments = AssessmentAttempt.objects.filter(
            updated_at__date=date,
            status=AssessmentAttempt.Status.COMPLETED
        )
...
class UserEvalQuestionAttemptRepository:
    ...
    def fetch_attempted_questions(assessment_attempt):
        attempted_questions = UserEvalQuestionAttempt.objects.filter(
            ...
            status__in=[
                UserEvalQuestionAttempt.Status.ATTEMPTED,
                UserEvalQuestionAttempt.Status.EVALUATING,
            ],
        ).all()
    ...
    def fetch_evaluated_questions(assessment_attempt):
        attempted_questions = UserEvalQuestionAttempt.objects.filter(
            assessment_attempt_id=assessment_attempt.assessment_id,
            status__in=[UserEvalQuestionAttempt.Status.EVALUATED],
        ).all()
        return attempted_questions
    ...
    def fetch_user_question_attempt(user, question_id, assessment_attempt):
        user_attempt = UserEvalQuestionAttempt.objects.filter(
            user_id=user.id,
            question_id=question_id,
            assessment_attempt_id=assessment_attempt.assessment_id,
        ).first()
...
class DSASheetConfigRepository:
    ...
    def get_all_configs():
        return [
            ...
            for config in DSASheetsConfig.objects.all()
        ]
...
class EventFlowRepository:
    ...
    def get_event_flows_for_assessment_ids(assessment_ids):
        return EventFlow.objects.filter(
            root_arguments__assessment_attempt_id__in=assessment_ids
        )
