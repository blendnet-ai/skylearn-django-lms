    ...
    def get_assistant(name):
        ...
        try:
            ...
            if assistant_from_db.assistant_id == "":
                ...
                new_assistant = OpenAIService().get_assistant(
                    id=assistant_from_db.assistant_id
                )
                return new_assistant
...
class ValidPromptTemplates:
    ...
    DSA_APPROACH = "dsa_approach_prompt"
...
class LLMCommunicationWrapper:
    ...
    def _send_user_message(self, user_msg: str, context_vars=None) -> str:
        ...
        filtered_context_vars = {
            key: value
            for key, value in context_vars.items()
            if key in self.prompt_template.logged_context_vars
        }
        # self.update_chat_history(context_vars)
        ...
        message = self.client.beta.threads.messages.create(
            thread_id=self.thread_id, role="user", content=user_msg
        )
    ...
    def send_user_message_and_get_stream_response_from_assistant(
        self,
        user_msg: str,
        event_handler,
        context_vars=None,
    ...
    def send_user_message_and_get_response_from_assistant(
        self, user_msg: str, context_vars=None
    ) -> str:
        self._send_user_message(user_msg, context_vars)
        ...
        if run.status == "completed":
            ...
            raise RuntimeError(
                f"Run failed with error code: {run.last_error.code}, message: {run.last_error.message}"
            )
...
class KnowledgeRepositoryRepository:
    ...
    def create_knowledge_repository(
        type_of_repo, source_path, source_type, index_path, sas_token
    ):
