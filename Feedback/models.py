...
class FeedbackResponse(models.Model):
    ...
    course_feedback_entry = models.ForeignKey(CourseFormEntry, on_delete=models.CASCADE, null=False, blank=False)
...
def store_old_end_date(sender, instance, **kwargs):
    ...
    if instance.id:  # Ensure this is an update, not a new entry
        try:
            ...
            instance._old_end_date = old_instance.end_date  # Store old value in a temporary attribute
...
def create_feedback_entries(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        should_create = hasattr(instance, '_old_end_date') and instance._old_end_date != instance.end_date
